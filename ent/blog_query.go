...
func (bq *BlogQuery) QueryOwner() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, blog.OwnerTable, blog.OwnerColumn),
		)
...
func (bq *BlogQuery) WithOwner(opts ...func(*UserQuery)) *BlogQuery {
	query := (&UserClient{config: bq.config}).Query()
...
func (bq *BlogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Blog, error) {
	...
	if query := bq.withOwner; query != nil {
		if err := bq.loadOwner(ctx, query, nodes, nil,
			func(n *Blog, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BlogQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Blog, init func(*Blog), assign func(*Blog, *User)) error {
